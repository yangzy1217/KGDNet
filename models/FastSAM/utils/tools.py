...
def fast_show_mask(
    ...
    # Update show with vectorized indexing.
...
def fast_show_mask_gpu(
    ...
    annotation = annotation[sorted_indices]
    # Find the first nonzero mask index at each position.
    ...
    # Use index to select the mask batch for each position.
...
def crop_image(annotations, image_like):
    ...
    for _, mask in enumerate(annotations):
        if np.sum(mask["segmentation"]) <= 100:
            filter_id.append(_)
            continue
        bbox = get_bbox_from_mask(mask["segmentation"])  # Mask bounding box.
        cropped_boxes.append(segment_image(image, bbox))  # Save the cropped image.
...
def point_prompt(masks, points, point_label, target_height, target_width):  # NumPy processing.
